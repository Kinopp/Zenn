...
解答例1では、`ORDERS`テーブルを2回登場させ（`o1`と`o2`）、それらをぶつけることで「自分以外の同じ条件のデータ」を探しています。自己結合で重複を探す際、最も重要なのが`o1.order_id <> o2.order_id`という不一致条件です。

```mermaid
flowchart LR
    A["o1.order_id <> o2.order_id
    がない場合"] --> B["すべての注文が
    「自分自身」と一致
    → 全行が結果に出てしまう"]
    C["o1.order_id <> o2.order_id
    がある場合"] --> D["「顧客IDと年月は同じだが
    注文IDは違う」
    → 他にも注文がある証明になる"]
```

これがないと、すべての注文が「自分自身（`e1.ID` = `e2.ID`）」と結合してしまい、すべての行が結果に出てきてしまいます。「顧客IDと年月は同じだが、注文IDは違う」という条件を加えることで、初めて「他にも注文がある（＝複数注文している）」ことを証明できます。

| アプローチ | 特徴 |
| :--- | :--- |
| 例1：自己結合 | 直感的で1つのクエリで完結。3回以上注文があると重複表示の可能性あり |
| 例2：GROUP BY＋結合 | 先に「2回以上注文した人」のリストを作ってから結合。ロジックが安定し大量データでも安定しやすい |
...
例1では、なぜ単純な`INNER JOIN`ではなく`LEFT OUTER JOIN`が必要なのか、という点がポイントです。

```mermaid
flowchart LR
    A["INNER JOINの場合"] --> B["結合相手が
    見つからない
    「ユニーク」な従業員は
    結果から消えてしまう"]
    C["LEFT OUTER JOINの場合"] --> D["右側(e2)に一致がなくても
    左側(e1)の全メンバーを
    確実に残す"]
```

もし`INNER JOIN`を使ってしまうと、名前が誰とも被っていない「ユニーク」な従業員は結合相手が見つからず、結果から消えてしまいます。`LEFT OUTER JOIN`を使うことで、右側（e2）に一致する人がいなくても左側（e1）の全メンバーを確実に残すことができます。

`COUNT(*)`ではなく、結合相手の列名`e2.employee_id`を指定してカウントしているのもポイントです。

| ケース | e2側の状態 | `COUNT(e2.employee_id)` |
| :--- | :--- | :---: |
| 重複がある | e2にデータがある | その人数がカウントされる |
| ユニーク | e2側がすべてNULL | 0（NULLはカウントされない） |

これによって「0＝ユニーク」という判定ができます。なお、対象範囲は`e1.employee_id BETWEEN 150 AND 165`で絞っていますが、比較相手の`e2`側には範囲の絞り込みがないため、対象範囲外の従業員と名前が一致した場合もカウントに含まれる点には注意しておくとよいと思います。
...
実務での使い分けとしては、自己結合（例1）はテーブルを2回読むためやや処理が重くなりやすい一方、古いバージョンのデータベースでも動くという利点があります。分析関数（例2）はロジックが直感的で、1回のスキャンで済むことが多く処理も速い傾向にありますが、Oracle 8i以降など比較的新しいバージョンが前提になります。

| 方式 | 処理の重さ | 対応バージョン |
| :--- | :--- | :--- |
| 自己結合（例1） | テーブルを2回読むためやや重い | 古いバージョンでも動く |
| 分析関数（例2） | 1回のスキャンで済み処理が速い傾向 | Oracle 8i以降が前提 |

現代の実務では、分析関数を使いこなせると、自己結合で書くと複雑になりがちな処理をすっきり書けることが多く、覚えておいて損はないと思います。
...
```mermaid
flowchart TD
    subgraph M1["例1：CROSS JOINで土台を作る"]
        direction TB
        A1["target_months（12行）"] -->|"CROSS JOIN"| A2["部門ID（2行）"]
        A2 --> A3["12×2=24行の空の器
        （マスター表）"]
        A3 -->|"LEFT OUTER JOIN"| A4["実績データを紐付け
        NVL(cnt,0)でゼロ埋め"]
    end
    subgraph M2["例2：PARTITION BYで複製する"]
        direction TB
        B1["target_months（12行）"] -->|"LEFT OUTER JOIN
        emp_counts
        PARTITION BY(department_id)"| B2["部門IDごとに
        カレンダーを内部で複製"]
        B2 --> B3["各部門12ヶ月分が
        自動的に生成される"]
    end
...
| 方式 | 汎用性 | 事前準備 |
| :--- | :--- | :--- |
| 例1：CROSS JOIN | 他DBでも応用可能 | 部門IDリストを別途用意する必要がある |
| 例2：PARTITION BY | Oracle専用の拡張構文 | 部門IDのリストを別途作らなくても自動で複製される |
