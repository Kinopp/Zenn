...
ご指摘ありがとうございます。13章はまだ`MATCH_RECOGNIZE`に触れていない段階なので、「思い出してください」「学びました」という過去形の言い回しは矛盾していました。第16章の内容は「まだ読者が知らない、この後登場する話」として先取り紹介する未来形の表現に修正します。該当箇所以外（問題文・期待結果・解答例）はそのまま維持しています。
