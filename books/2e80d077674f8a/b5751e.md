...
### 難易度：★★☆☆☆ (Lv.2)
## 問題
マーケティング部門より、「クリケット（`PROD_CATEGORY` = 'Cricket'）カテゴリの製品と比較して、価格面で優位性のある製品」をリストアップしたいという依頼がありました。
`SH.PRODUCTS`テーブルを使用し、以下の2つの観点でそれぞれ製品を抽出してください。

**【抽出・編集ルール】**
* **観点A**：Cricketカテゴリの製品のうち、**どれか1つでも**下回っている（＝自分の方が高い）製品を`ANY`演算子を用いて抽出する。
* **観点B**：Cricketカテゴリの**すべての**製品より高額な（＝Cricket製品の中で最も高いものすら上回る）製品を`ALL`演算子を用いて抽出する。
* Cricketカテゴリ自身の製品は比較対象から除外すること（`PROD_CATEGORY <> 'Cricket'`）。
* 表示項目：`PROD_CATEGORY`, `PROD_NAME`, `PROD_LIST_PRICE`
* 結果は価格（`PROD_LIST_PRICE`）の降順で表示すること。
...
| Soccer / Football | Soccer Goal - Official | 1099.99 |
| Baseball | Pitching Machine and Batting Cage Combo | 999.99 |
| Golf | Right-Handed Graphite Shaft Iron Set | 999.99 |
| Baseball | Speed Trainer Bats and Training Program | 899.99 |
| Soccer / Football | Soccer Goal - Portable | 899.99 |
| Tennis | Match Used Autograph Racquet | 599.99 |
| Tennis | Limited Edition Racquet | 499.99 |
| Tennis | Sonic Core Graphite Racquet | 299.99 |
| Baseball | Pro Maple Bat | 192.99 |

（以下、36.99円〜7.99円の製品まで続き、合計46件）
...
FROM
    sh.products p
WHERE
    p.prod_list_price > ANY (
        SELECT sub.prod_list_price
        FROM sh.products sub
        WHERE sub.prod_category = 'Cricket'
    )
    AND p.prod_category <> 'Cricket'
ORDER BY
    p.prod_list_price DESC
```
```sql:例2：ALLを使用
SELECT
    p.prod_category,
...
## NG例（一見正しそうで、実は意図と異なる結果になる例）
```sql:NG例：「Cricketの誰よりも高い」つもりでANYを使ってしまう
SELECT
    p.prod_category,
    p.prod_name,
    p.prod_list_price
...
ORDER BY
    p.prod_list_price DESC
```
このNG例は、コメントや変数名だけを見ると「Cricket製品よりも高額な、真の高級品リスト」を作っているように見えます。しかし実際に実行すると46件もヒットし、7.99円のテニスグリップまで含まれてしまいます。「Cricketカテゴリの製品よりANYで比較する」という言葉の響きから「Cricket全体より優れている」という意味に読み違えやすいのですが、実際は「Cricketの中で**最も安い**製品」を超えていれば条件を満たしてしまうためです。エラーにはならず、件数が想定よりずっと多いことに気づかない限り、この間違いは発覚しません。

## 解説
`ANY`（`SOME`）は`ALL`と対になる複数行比較演算子ですが、名前の直感とは裏腹に、初学者が最も誤解しやすい構文の1つです。
...
`p.prod_list_price > ANY (...)`は、「サブクエリが返す値の**少なくとも1つ**より大きければ真」という意味です。Cricketカテゴリの中に数百円台の安価な備品（グリップテープ等）が含まれているため、それより高い製品（＝ほぼすべての球技用品）が条件を満たしてしまい、結果として46件という広範なリストになりました。つまり`> ANY`は実質的に「**サブクエリの最小値**より大きいか」とほぼ同義であり、`> ANY (SELECT ... )`は`> (SELECT MIN(...) FROM ...)`と書き換えてもほぼ同じ結果になります。

一方`p.prod_list_price > ALL (...)`は、「サブクエリが返す**すべての値**より大きければ真」という意味です。Cricketカテゴリの最高額（199.99円）を上回る製品だけに絞られるため、結果は9件という、より厳選されたプレミアム製品リストになりました。こちらは実質的に「**サブクエリの最大値**より大きいか」と同義であり、問題9-6で登場した`>= ALL`（最新日付の判定）と同じ発想です。

| 演算子 | 論理的な意味 | 近い書き換え | 直感的に誤解しやすい点 |
| :--- | :--- | :--- | :--- |
| `> ANY` | 少なくとも1つより大きい | `> (SELECT MIN(...))` | 「ANY＝みんな」と誤読し、`ALL`のつもりで使ってしまう |
| `> ALL` | すべての値より大きい | `> (SELECT MAX(...))` | こちらが「全部より上」の意味だが、名前だけでは判別しにくい |
| `= ANY` | いずれかと等しい | `IN (...)` | `IN`と完全に同義（`SOME`も同様） |

`= ANY`は`IN`演算子と全く同じ意味になるため、実務でわざわざ`= ANY`と書く場面は少なく、`ANY`が使われるのはほとんどの場合`>`, `<`のような不等号との組み合わせです。なお`SOME`は`ANY`の同義語（シノニム）であり、動作に違いはありません。可読性の観点から「一部でも該当すれば」というニュアンスを強調したいときに`SOME`を選ぶ書き手もいますが、Oracleの現場では`ANY`表記の方が一般的です。

今回のNG例のように、`ANY`と`ALL`の取り違えはSQLエラーにはならず、**件数が想定よりずっと多い（あるいは少ない）** という形で静かに現れます。今回の実測値（ANYで46件、ALLで9件）を見ても分かる通り、その差は決して小さくありません。「◯◯より高い」という要件を見たら、それが「どれか1つでも上回ればOK（ANY）」なのか「全部を上回る必要がある（ALL）」なのか、日本語の要件定義の段階で曖昧さを潰しておくことが重要です。

観点Bの9件を見ると、いずれもGolf・Soccer/Football・Baseball・Tennisの上位モデルばかりであり、Cricket用品（バット等）の最高額199.99円という水準がいかに低く、他の球技の本格装備がいかに高額かが、このリストからも読み取れます。

:::message
### ANY/ALLよりMIN/MAXの方が実務では読みやすいことが多い
ここまで見てきた通り、`> ANY (...)`は`> (SELECT MIN(...) FROM ...)`と、`> ALL (...)`は`> (SELECT MAX(...) FROM ...)`と、単一行サブクエリ（問題9-2）に書き換えることができます。

実務では、`ANY`/`ALL`よりも`MIN`/`MAX`を使った単一行サブクエリの方が好まれる傾向にあります。理由は主に2つあります。1つ目は可読性で、`> ANY`や`> ALL`は読み手が一度立ち止まって論理を再構築する必要がありますが、`> (SELECT MAX(...))`であれば「最大値より大きいか」と一目で意味が伝わります。2つ目はパフォーマンスで、`MIN`/`MAX`を使った書き方はオプティマイザが集計値を1つに確定させてから比較するため、実行計画が安定しやすいという実務上の経験則があります。

とはいえ、`ANY`/`ALL`はSQL標準に準拠した書き方であり、他のRDBMS（PostgreSQLやSQL Serverなど）へ移植する際にも通用する記法です。「読めるが罠がある構文」として理解しておき、実際に書くときは`MIN`/`MAX`に置き換えられないか一度検討してみる、というスタンスがおすすめです。
:::

---
<br><br>

# 【完全版】問題9-12：関係除算（二重NOT EXISTSによる「すべて満たす」判定）
### 難易度：★★★★☆ (Lv.4)

## 問題
マーケティング部門より、「ゴルフ入門3点セット（クラブ・ボール・グローブ）を**すべて**購入したことのある顧客」を特定してほしいという依頼がありました。
以下のWITH句で用意したサンプルデータ（`target_products`：対象3製品、`customers_sample`：顧客マスタ、`purchase_log`：購入履歴）を使用し、3製品を**もれなく**購入している顧客を抽出してください。

**【抽出・編集ルール】**
* 対象3製品（P001, P002, P003）を、それぞれ最低1回は購入している顧客のみを抽出すること。
* 対象外の製品（例：P004）を追加で購入していても構わない（3点セット以外も買っている顧客も対象に含める）。
* 同じ製品を複数回購入している履歴が含まれる可能性がある点に注意すること。
* 表示項目：`CUST_ID`, `CUST_NAME`
* 結果は`CUST_ID`の昇順で表示すること。

```sql:検証用データ
WITH target_products AS (
    SELECT 'P001' AS prod_id, 'ゴルフクラブ'   AS prod_name FROM DUAL UNION ALL
    SELECT 'P002',            'ゴルフボール'   FROM DUAL UNION ALL
    SELECT 'P003',            'ゴルフグローブ' FROM DUAL
),
customers_sample AS (
    SELECT 'C001' AS cust_id, '田中太郎' AS cust_name FROM DUAL UNION ALL
    SELECT 'C002',            '鈴木一郎'            FROM DUAL UNION ALL
    SELECT 'C003',            '佐藤花子'            FROM DUAL UNION ALL
    SELECT 'C004',            '高橋次郎'            FROM DUAL
),
purchase_log AS (
    -- C001：3点セットをきっちり1回ずつ購入
    SELECT 'C001' AS cust_id, 'P001' AS prod_id FROM DUAL UNION ALL
    SELECT 'C001',            'P002'            FROM DUAL UNION ALL
    SELECT 'C001',            'P003'            FROM DUAL UNION ALL
    -- C002：P001を2回購入（重複）だが、P003は未購入
    SELECT 'C002',            'P001'            FROM DUAL UNION ALL
    SELECT 'C002',            'P002'            FROM DUAL UNION ALL
    SELECT 'C002',            'P001'            FROM DUAL UNION ALL
    -- C003：3点セット＋対象外のP004も購入
    SELECT 'C003',            'P001'            FROM DUAL UNION ALL
    SELECT 'C003',            'P002'            FROM DUAL UNION ALL
    SELECT 'C003',            'P003'            FROM DUAL UNION ALL
    SELECT 'C003',            'P004'            FROM DUAL UNION ALL
    -- C004：P001のみ購入
    SELECT 'C004',            'P001'            FROM DUAL
)
-- ここに解答のSELECT文を続ける
```
...
WHERE
    ...
    NOT EXISTS (
        ...
        WHERE NOT EXISTS (
            ...
            WHERE pl.cust_id = c.cust_id
              AND pl.prod_id = tp.prod_id
        )
    )
ORDER BY
    c.cust_id
```
```sql:例2：GROUP BY + HAVING COUNT(DISTINCT)を使用
WITH target_products AS (
    SELECT 'P001' AS prod_id FROM DUAL UNION ALL
...
FROM
    customers_sample c
    INNER JOIN purchase_log pl ON pl.cust_id = c.cust_id
WHERE
    pl.prod_id IN (SELECT prod_id FROM target_products)
GROUP BY
    c.cust_id, c.cust_name
HAVING
    -- 重複購入を無視し、「対象製品のうち何種類買ったか」で判定
    COUNT(DISTINCT pl.prod_id) = (SELECT COUNT(*) FROM target_products)
ORDER BY
    c.cust_id
```

## NG例（一見正しそうで、実は意図と異なる結果になる例）
```sql:NG例：COUNT(DISTINCT)ではなくCOUNT(*)で判定してしまう
SELECT
    c.cust_id,
    c.cust_name
FROM
...
GROUP BY
    c.cust_id, c.cust_name
HAVING
    COUNT(*) = (SELECT COUNT(*) FROM target_products)  -- DISTINCTが抜けている
ORDER BY
    c.cust_id
```
このNG例は構文的に完全に正しく、エラーも出ません。「対象製品の件数（3件）と、購入履歴の件数が一致すればセット購入者だろう」という一見自然な発想ですが、購入履歴に**重複行**が含まれるとロジックが崩れます。

C002は、P001を2回・P002を1回購入しており、`purchase_log`との結合後の行数は3件です。`COUNT(*) = 3`という条件に一致してしまうため、本来はP003を未購入で対象外のはずのC002が、**誤って抽出されてしまいます**。逆にC003は、3点セットに加えてP004も購入しているため、結合後の行数は3件（P001, P002, P003が対象製品としてマッチ）にとどまり一見問題なさそうですが、もし対象外製品を含む形でJOIN条件を誤ると行数がズレるなど、`COUNT(*)`は「何行あるか」しか見ておらず「何種類の対象製品があるか」を保証しないという弱点を抱えています。

`DISTINCT`を1つ書き忘れるだけで、件数がたまたま一致する顧客だけが静かに紛れ込む、あるいは本来該当するはずの顧客が漏れる、という危険なパターンです。

## 解説
今回のテーマは、リレーショナルデータベース理論における古典的な難問「関係除算（Relational Division）」です。「Aを満たす顧客のうち、Bの条件を“すべて”満たす顧客を探す」という要件は、実務でも「必修科目をすべて履修した学生」「必要な資格をすべて保有する社員」「セット商品の構成品をすべて買った顧客」など、姿を変えて頻繁に登場します。

問題9-4では「特定の条件に合うデータが1件も存在しないこと」を`NOT EXISTS`で判定しましたが、今回はその応用として、`NOT EXISTS`を **入れ子（二重）** にします。

```mermaid
flowchart TD
    A["顧客を1人ずつ見る"] --> B{"外側のNOT EXISTS：
    「この顧客について、
    未購入の対象製品が
    1つでも存在するか」"}
    B --> C{"内側のNOT EXISTS：
    ...
    購入した記録があるか」"}
    C -->|"購入記録なし
    （＝未購入の製品が見つかった）"| D["内側NOT EXISTS = TRUE
    → 外側もTRUE
    → その顧客は除外"]
    C -->|"全対象製品について
    購入記録が見つかった"| E["内側NOT EXISTS = FALSE
    → 外側もFALSE
    → その顧客は残る"]
```

論理を日本語に開くと、「対象製品の中に、この顧客が**買っていないもの**が1つでもあれば（内側の`NOT EXISTS`が真になれば）、その顧客を除外する」という二重否定の構造になっています。「未購入の製品が1つも存在しない」＝「すべての対象製品を購入している」という、数学の論理学における「∀（すべての）」を、SQLの`NOT EXISTS`の入れ子で表現しているのが例1の本質です。

C001は3製品すべてに購入記録があるため、内側の`NOT EXISTS`はどの製品についても「購入記録なし」を見つけられず、常にFALSEとなり、外側の`NOT EXISTS`もFALSE、つまり除外されずに残ります。C002はP003の購入記録がないため、内側の`NOT EXISTS`がP003についてTRUEとなり、外側もTRUE、つまり除外されます。C003は対象外のP004も買っていますが、それは判定に一切影響しません。3点セットさえ揃っていれば合格です。

例2の`GROUP BY` + `HAVING COUNT(DISTINCT)`は、同じ関係除算をもう1つの発想で解いたものです。「対象製品でJOINし、顧客ごとに何種類の対象製品が見つかったかを数え、その種類数が対象製品の総数と一致すれば、全部買っている」という考え方です。

| 方式 | 考え方 | 向いている場面 |
| :--- | :--- | :--- |
| 二重`NOT EXISTS`（例1） | 「未購入の製品が1つもない」ことを直接判定 | 対象製品数が少ない、または存在判定を明示的に書きたいとき |
| `GROUP BY` + `HAVING COUNT(DISTINCT)`（例2） | 「一致した種類数」を数えて対象数と比較 | 集計・件数ベースで考えたいとき。ただし`DISTINCT`の付け忘れに注意 |

例2は直感的で書きやすい一方、NG例で見た通り`COUNT(DISTINCT ...)`の`DISTINCT`を書き忘れると、重複購入という現実によくあるデータパターンに対して脆くなります。これに対し例1の二重`NOT EXISTS`は「存在するかしないか」だけを見ているため、同じ製品を何回買おうが判定に一切影響せず、重複データに対して構造的に頑健です。関係除算を書く際は、まず二重`NOT EXISTS`で正しいロジックを組み立て、可読性や実行計画の都合で`GROUP BY`版に書き換える、という順序がおすすめです。

:::message
### 「すべて」を扱うSQLは、否定の二重化で表現する
SQL（およびリレーショナル代数）には、英語の"for all"や日本語の「すべて」を直接表現する演算子が用意されていません。その代わりに使われるのが、論理学の\` ∀x P(x) ⟺ ¬∃x ¬P(x) \`（「すべてのxについてPが成り立つ」は「Pが成り立たないxが存在しない」と等価）という関係です。

問題9-4で学んだ`NOT EXISTS`は「1つも存在しない」を表現する道具でしたが、今回はそれをもう1段階かぶせることで、「（Pが成り立たないものが）1つも存在しない」＝「すべてでPが成り立つ」を表現しています。この二重否定の考え方は最初は直感に反して難しく感じますが、「関係除算＝二重NOT EXISTS」というパターンとして丸ごと覚えてしまうと、いざ「すべて満たす」という要件に出会ったときに迷わず対応できるようになります。
:::

---
<br><br>

# 【完全版】問題9-13：WITH句のマテリアライズ制御（`INLINE` / `MATERIALIZE`ヒント）
### 難易度：★★★★☆ (Lv.4)

## 問題
マーケティング部門が、キャンペーン対象者を50%の確率でランダムに振り分ける抽選処理を作成しています。「同じ顧客であれば、抽選結果（当選フラグ）は当然1つに定まるはず」という前提のもと、抽選結果を2つの集計（当選者リストと、当選者数のサマリー）で使い回したいと考えています。

以下のWITH句を使い、顧客ごとに1回だけ抽選を行い、その結果を**自己結合で突き合わせて、同じ顧客なのに結果が矛盾していないか**を検証してください（本来は0件になるはずです）。

```sql:検証用データ
WITH customers_sample AS (
    SELECT LEVEL AS cust_id FROM DUAL CONNECT BY LEVEL <= 30
)
-- ここに抽選用CTEと検証クエリを続ける
```

**【条件およびルール】**
* `DBMS_RANDOM.SEED(42)`を検証クエリの直前に実行し、乱数を固定してください（再現性確保のため）。
* 顧客ごとに`DBMS_RANDOM.VALUE < 0.5`で当選（'Y'）／落選（'N'）を判定するCTE（`lottery_result`）を作成する。
* `lottery_result`を**自己結合**し、同一`cust_id`同士で当選フラグが**一致しない**行を抽出する（本来は0件のはず）。
* 表示項目：`CUST_ID`, `A.FLAG`（1回目の参照）, `B.FLAG`（2回目の参照）

## 解答例
```sql:NG例：INLINEヒントにより、CTEが参照のたびに再評価されてしまう
BEGIN
    DBMS_RANDOM.SEED(42);
END;
/
...
lottery_result AS (
    SELECT /*+ INLINE */
        cust_id,
        CASE WHEN DBMS_RANDOM.VALUE < 0.5 THEN 'Y' ELSE 'N' END AS flag
    FROM
        customers_sample
)
SELECT
...
FROM
    lottery_result a
    INNER JOIN lottery_result b ON a.cust_id = b.cust_id
WHERE
    a.flag <> b.flag
ORDER BY
    a.cust_id;
```
```sql:正しい例：MATERIALIZEヒントにより、CTEを1回だけ評価して結果を再利用する
BEGIN
    DBMS_RANDOM.SEED(42);
END;
/
...
NG例の実行結果は乱数の消費順序に依存するため断定はできませんが、**理論上、0件にならない（矛盾する行が1件以上現れうる）** 点がこの問題の核心です。一方、正しい例（`MATERIALIZE`）は、CTEが1回しか評価されないため、`a.flag`と`b.flag`は常に一致し、**必ず0件**になります。

## 解説
問題9-1でWITH句（副問い合わせファクタリング句）の基本を扱いましたが、そこでは「WITH句は使い回せる名前付きサブクエリ」という説明にとどめていました。実は、WITH句で定義した名前が**クエリ内で複数回参照された場合**、Oracleのオプティマイザは2つの戦略のどちらかを選びます。

```mermaid
flowchart TD
    A["WITH句で定義した
    lottery_result"] --> B{"オプティマイザの選択"}
    B -->|"INLINE
    （展開・インライン化）"| C["参照される都度、
    定義元のSELECTが
    再実行される"]
...
`INLINE`（インライン展開）は、WITH句の定義をあたかも通常のサブクエリのように、参照箇所へそのまま展開してしまう戦略です。今回の`lottery_result`が2箇所（`a`, `b`）から参照されているため、インライン展開されると`SELECT ... CASE WHEN DBMS_RANDOM.VALUE < 0.5 ...`という処理自体が**2回別々に実行**されます。`DBMS_RANDOM.VALUE`は呼び出すたびに違う値を返す非決定的な関数のため、同じ`cust_id`のはずなのに1回目の呼び出しと2回目の呼び出しで異なる乱数が使われ、当選フラグが食い違ってしまう可能性が生まれます。

一方`MATERIALIZE`（実体化）は、WITH句の結果を一時的な作業領域（グローバル一時表に近いイメージ）に一度だけ書き出し、以降の参照はすべてその保存済みの結果を読みに行きます。`DBMS_RANDOM.VALUE`は1回しか呼ばれないため、`a`から見ても`b`から見ても、同じ`cust_id`は必ず同じフラグになります。

| ヒント | 評価回数（今回のケース） | 非決定的な処理との相性 |
| :--- | :--- | :--- |
| `INLINE` | 参照のたびに再評価（今回は2回） | ❌ 危険：呼び出しごとに結果が変わりうる |
| `MATERIALIZE` | 最初の1回のみ | ⭕ 安全：一貫した結果が保証される |

今回はあえて`DBMS_RANDOM.VALUE`という非決定的な関数を使うことで、CTEの複数回評価という普段は目に見えない現象を、目に見える形（本来ありえない自己結合の不一致）として可視化しました。実務でこの問題が顕在化するのはむしろ「決定的だが重い処理」を含むCTEを何度も参照するケースです。例えば、複雑な集計を含むCTEを`SELECT`と`WHERE`の両方から参照していると、インライン展開によって同じ集計処理が2回走り、結果は正しくても**実行時間が想定の2倍かかる**、というパフォーマンス上の問題として現れます。結果が正しいだけに、実行計画（`EXPLAIN PLAN`）を確認しない限り、この無駄な二重実行に気づきにくいという点で、これもまた一種の「静かな失敗」と言えます。

Oracle 12c以降、WITH句が複数回参照されている場合はオプティマイザが自動的に`MATERIALIZE`を選びやすいという経験則がありますが、これはあくまでコストベースの判断であり、常に保証されるわけではありません。挙動を明示的に制御したい場合は、今回のように`/*+ MATERIALIZE */`ヒントをCTE定義の`SELECT`直後に書くことで、意図を確実にオプティマイザへ伝えることができます。

:::message
### 非決定的な関数をWITH句に含めるときは、必ずMATERIALIZEを検討する
`DBMS_RANDOM`だけでなく、`SYSDATE`、`SYSTIMESTAMP`、シーケンスの`NEXTVAL`など、呼び出すたびに結果が変わりうる非決定的な処理をWITH句に含める場合、そのCTEが複数箇所から参照される可能性があるなら`/*+ MATERIALIZE */`を明示することを強く推奨します。

特に`NEXTVAL`は要注意です。もしWITH句内で採番したシーケンス値を複数箇所で使い回すつもりが、インライン展開によって参照のたびに`NEXTVAL`が実行されてしまうと、採番がどんどん進んでしまい、「同じはずの連番が、参照箇所によって値がズレる」という、今回のケース以上に発見しづらいバグにつながります。「WITH句は名前を付けているだけで、実体は必ずしも1つに固定されない」ということを覚えておいてください。
:::
