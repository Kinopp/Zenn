...
今回のクエリでは、4つのテーブルが連なるように結合されています。`ORDERS`（注文）が全体の中心で、誰がいつ注文したかを管理し、`CUSTOMERS`（顧客）が`customer_id`で`ORDERS`と結合されて顧客名を、`ORDER_ITEMS`（注文明細）が`order_id`で`ORDERS`と結合されて注文の中身を、`PRODUCT_INFORMATION`（商品情報）が`product_id`で`ORDER_ITEMS`と結合されて商品名を提供しています。

解答例には3つの書き方があり、実務での評価は大きく異なります。例1の`INNER JOIN ... ON`は最も標準的で現在主流の書き方で、「どのテーブルとどのテーブルを、何の条件でくっつけるか」が1セットで書けるため読みやすく、ほとんどのプロジェクトで推奨されます。例2のカンマ区切り＋`WHERE`句は、Oracleの古いバージョンでよく使われていた書き方です。結合条件（JOIN）と抽出条件（WHERE）が混ざり合うため、テーブルが増えると「結合し忘れ（デカルト積）」という事故を招きやすく、既存の古いシステムの保守で見かけることはあっても、新規で書くなら例1を使うのが無難です。例3の`USING`句は、結合するカラム名が両方のテーブルで全く同じ（例：両方とも`order_id`）場合に使える書き方で、`oi.order_id = o.order_id`と書く手間が省ける一方、テーブル設計によってはカラム名が違うこともあるため、汎用性は`ON`の方が高いです。
...
また`JOIN`で`ON`ではなく`USING`を使う場合、逆にテーブル修飾をしてはいけないという特殊なルールがあります。`SELECT E.DEPT_ID FROM EMPLOYEES E JOIN DEPARTMENTS D USING (DEPT_ID)`はNGで、`SELECT DEPT_ID FROM EMPLOYEES E JOIN DEPARTMENTS D USING (DEPT_ID)`が正解です。`USING`を使うと、Oracle内部でその列が「結合キー」として1つに統合されるため、修飾すると逆に不適切とみなされます。
...
外部結合（OUTER JOIN）には、「どのテーブルを主役（全件表示）にするか」によって3つの種類があります。左外部結合（LEFT OUTER JOIN）は左側に書いたテーブルのデータをすべて表示し、右側のテーブルに一致するデータがあればくっつけます（先に登場したテーブルを「左」、後に登場したテーブルを「右」と表現します）。実務で最もよく使われる結合です。
...
判断基準としては、「結合相手がいないレコードは表示されないのが正しいか」を自問してみるとよいと思います。いない（存在しなければ無価値）ならINNER JOIN、いなくても存在自体を表示したいならOUTER JOIN、という具合です。注文詳細のない注文や、社員のいない部署はINNER JOINで消してよいケースが多く、売上が0円の商品一覧やまだ配属先が決まっていない新入社員はOUTER JOINで残すべきケースにあたります。
...
`FULL OUTER JOIN`は、左右両方のテーブルにあるデータを1つも漏らさずに結合します。両方のテーブルにある国（例：JP, USなど）は1行にまとまって表示され、HRにはあるがSHにない国はSH側の列がNULLになり、SHにはあるがHRにない国はHR側の列がNULLになります。今回の目的は、このうち「片方だけがNULLになっている」不一致データ（対称差）を抜き出すことです。

解答例は2つのアプローチを示しています。UNION ALLを使う方法（例1）は、「SHにないリスト」と「HRにないリスト」を別々に作って最後に縦に結合する方法です。「上半分はSHなし担当、下半分はHRなし担当」と役割が分かれているため、ロジックが明快で後から修正しやすいのが特徴です。CASE式を使う方法（例2）は、1つの`SELECT`文の中で条件分岐を使って表示を切り替える方法で、クエリがコンパクトにまとまります。`hr IS NULL`ならその国は`sh`側から来たデータなのでステータスを「HRになし」に、そうでなければ（`sh IS NULL`なら）「SHになし」にする、という流れです。
...
## 解説
今回は自己結合の応用編です。単にデータを繋げるだけでなく、「同じテーブル内のデータ同士を比較して、重複や特定のパターンを見つけ出す」という、少し高度なロジックが求められます。実務では「同じ日に2回以上来店した客」や「二重登録されている可能性のあるデータ」の抽出など、データクレンジングや不正検知でよく使われるパターンです。

解答例1では、`ORDERS`テーブルを2回登場させ（`o1`と`o2`）、それらをぶつけることで「自分以外の同じ条件のデータ」を探しています。自己結合で重複を探す際、最も重要なのが`o1.order_id <> o2.order_id`という不一致条件です。これがないと、すべての注文が「自分自身」と一致してしまい、すべての行が結果に出てきてしまいます。「顧客IDと年月は同じだが、注文IDは違う」という条件を加えることで、初めて「他にも注文がある（＝複数注文している）」ことを証明できます。

例1の自己結合（Self-Join）は、1つの注文に対し同じ人の同じ年月の「別の注文」を横に並べようと試みる、直感的な書き方です。副問い合わせを使わないため1つのクエリの流れで完結する一方、同じ月に3回注文した人がいる場合、A-B、A-Cのように1つの注文IDが複数回ヒットして重複表示される可能性があり、実務では`DISTINCT`を併用することもあります。

例2の集計（GROUP BY）＋結合は、より確実で読みやすいとされるアプローチです。まずWITH句（`o1`）で注文テーブルを「顧客・年・月」で集計し、`HAVING COUNT(*) > 1`を使って「2回以上注文した人」というターゲットリストを先に作ります。そのターゲットリストを元の注文テーブル（`o2`）と結合することで、最初に「どの月のどの客か」を確定させてから詳細（注文ID）を取りに行くため、ロジックが安定しています。大量データを扱う場合は、こちらの方がパフォーマンスも安定しやすい傾向にあります。

年月を比較するために使われている`EXTRACT`関数は、第7章の日付セクションでも扱いました。

```sql
extract(YEAR FROM o1.order_date) = extract(YEAR FROM o2.order_date)
```

これを`TO_CHAR(order_date, 'YYYYMM')`で比較しても正解ですが、`EXTRACT`を使うと数値として比較されるため、内部の処理効率が良くなる場合があります。

Matthias MacGrawさん（105番）を見てください。2008年1月8日の注文（2358）と2008年1月26日の注文（2356）、同じ「2008年1月」の中に異なる注文IDが2つ存在していることが分かります。このようなリストは、マーケティング部門から「同じ月に何度も買ってくれるロイヤルカスタマーにクーポンを送りたい」といった要望でよく作成されます。その際、さらに`SUM(order_total)`などを加えて「月間合計額」も出すと、より使えるレポートになります。
...
動作としては、左側の`hr.countries`から地域20の国（5件）を取得し、右側の`WITH`句で作った`t_year`（2件：2020年と2021年）を用意し、この2つを掛け算することで5カ国×2年＝10行の結果が生成されます。もし地域20に100カ国あれば200行になります。このように、データを増やして網羅的な表を作るのがクロス結合の特徴です。
...
`e1.employee_id < e2.employee_id`という条件には、2つの役割があります。1つ目は自分自身との一致を除外する役割です。この条件がないと、すべての行が「自分自身（`e1.ID` = `e2.ID`）」と結合してしまい、全員が結果に出てきてしまいます。2つ目は重複ペアの「裏返し」をカットする役割です。例えば、ID:103のAlexanderとID:115のAlexanderがいる場合、もし条件を`e1.ID <> e2.ID`（等しくない）だけにすると、「ID:103とID:115」「ID:115とID:103」の2行が出てしまいます。「片方のIDがもう片方より小さい」という不等号（`<`）を使うことで、ペアを1つに固定し、すっきりしたリストにしています。
...
## 解説
これは実務で行う「名寄せ」や「データ品質調査」の集大成とも言える問題です。「重複がある人だけ」を出すのは問題10-7で扱いましたが、今回は「重複がない人（ユニークな人）」も含めて全員をリストアップし、それぞれの状態をラベル付けするという、より踏み込んだロジックが求められています。

例1では、なぜ単純な`INNER JOIN`ではなく`LEFT OUTER JOIN`が必要なのか、という点がポイントです。もし`INNER JOIN`を使ってしまうと、名前が誰とも被っていない「ユニーク」な従業員は結合相手が見つからず、結果から消えてしまいます。`LEFT OUTER JOIN`を使うことで、右側（e2）に一致する人がいなくても左側（e1）の全メンバーを確実に残すことができます。

`COUNT(*)`ではなく、結合相手の列名`e2.employee_id`を指定してカウントしているのもポイントです。重複がある場合はe2にデータがあるのでその人数がカウントされ、ユニークな場合はe2側がすべてNULLなので`COUNT`は0になります。これによって「0＝ユニーク」という判定ができます。なお、対象範囲は`e1.employee_id BETWEEN 150 AND 165`で絞っていますが、比較相手の`e2`側には範囲の絞り込みがないため、対象範囲外の従業員と名前が一致した場合もカウントに含まれる点には注意しておくとよいと思います。

例2で使われている`COUNT(*) OVER(PARTITION BY ...)`は、分析関数（ウィンドウ関数）と呼ばれる機能です。通常の`GROUP BY`は行を一つにまとめてしまいますが、分析関数は「行の数はそのままに、集計結果だけを横に添える」ことができます。`PARTITION BY first_name`は「名前ごとにグループ（パーティション）を作って、その中で数を数えてね」という指示です。`COUNT(*) OVER(...)`で「自分を含めた同じ名前の総数」が出るので、そこから`- 1`することで「自分以外の重複者数」を導き出しています。

実務での使い分けとしては、自己結合（例1）はテーブルを2回読むためやや処理が重くなりやすい一方、古いバージョンのデータベースでも動くという利点があります。分析関数（例2）はロジックが直感的で、1回のスキャンで済むことが多く処理も速い傾向にありますが、Oracle 8i以降など比較的新しいバージョンが前提になります。現代の実務では、分析関数を使いこなせると、自己結合で書くと複雑になりがちな処理をすっきり書けることが多く、覚えておいて損はないと思います。

結果を見ると、Davidさんが「2人重複（＝合計3人）」、Peterさんが「1人重複（＝合計2人）」、そして他のメンバーがユニークとして抽出されています。例1では`e1.ID <> e2.ID`、例2では「全体数 - 1」と、アプローチは違えど「自分自身をカウントから外す」という一貫したロジックが貫かれています。これがデータの正確性を支える重要なポイントです。
...
このクエリは3つの部品で構成されています。まず`full_year_month`で、データの有無に関わらず「存在するべき年月のリスト」を抽出し、これがレポートの縦軸になります。次に`order_pr`で、実際に売れたデータだけを「年月×商品」で集計します。ここではまだ「売れた時」のデータしかありません。最後の`SELECT`句でパーティション外部結合を使い、「全ての年月（f）」に対して「商品ごとの枠組み（PARTITION BY）」を作り、そこに「実績（o）」を流し込みます。実績がなければNULLになるので、最後に`COALESCE(o.sm, 0)`で0に変換して完成です。
...
## 解説
実務で売上や採用の「月次推移グラフ」をBIツール（TableauやExcelなど）で作成するとき、データが0の月が行ごとスキップされてしまい、「折れ線グラフが途切れる」「あるはずの月が軸から消える」という現象に直面したことのあるエンジニアは多いと思います。

今回は、標準的なアプローチ（例1）と、Oracle独自の構文（例2）の2つが提示されています。それぞれの仕組みと実務での違いを見ていきます。

通常の`GROUP BY`は「テーブルに存在する行」しか集計できません。2015年5月に部門50の採用者が0人だった場合、テーブルにはレコード自体が存在しないため、SQLは「0人」という結果ではなく「5月の行そのものを出力しない」という挙動になります。これを解決するために、解答例ではまず`target_months`（12ヶ月分のカレンダーの土台）を動的に生成しています。

例1のCROSS JOIN（掛け算）によるアプローチは、他システム（MySQLやPostgreSQLなど）でも応用できる汎用的な方法です。`target_months`（12行）と対象の部門ID（2行）を`CROSS JOIN`して、あらかじめ「全24行（12ヶ月×2部門）」の空の器（マスター表）を作り、その器に対して実際の集計データ（`emp_counts`）を`LEFT OUTER JOIN`で紐付け、データがない場所を`NVL(cnt, 0)`でゼロ埋めします。

例2のパーティション・アウター結合（Oracle特有）は、器をわざわざCROSS JOINで作る必要がありません。
...
`PARTITION BY (e.department_id)`が今回のポイントで、「外部結合（`LEFT OUTER JOIN`）を実行するとき、部門IDごとにグループを小分けにして、それぞれのグループに対してカレンダー（`target_months`）を丸ごと1枚ずつ適用してね」という命令になります。

通常の外部結合（`LEFT JOIN`）では、結合元（左側）の行数は維持されますが、結合先（右側）にある「部門ID」のバリエーションをすべて網羅して複製することはできません。データのない月は`DEPARTMENT_ID`もNULLになってしまい、部門ごとの12ヶ月分は作れないのです。一方`PARTITION BY`付き`LEFT JOIN`は、指定した列（部門ID）の値の種類ごとにカレンダーを内部で複製し、それぞれに対して結合を試みます。結果として、実データがどれだけスカスカ（疎データ）であっても、指定した軸の数だけきれいにデータが引き伸ばされ、高密度な（密データ）表が完成します。

実務でこの「データの穴埋め」を行う際は、2点に注意するとよいと思います。1つ目は軸（パーティション）の絞り込み位置です。例2の`emp_counts`内で、あらかじめ`WHERE department_id IN (30, 50)`と絞り込んでいるのがポイントで、もしここで絞り込まないと、テーブル内に存在するすべての部門ID（10〜110など）に対して12ヶ月分のカレンダーが複製されてしまい、想定より大きな結果セットが生成されてしまいます。2つ目はNVL関数の適用で、穴埋めされた行の集計値はNULLになっているため、画面表示やCSV出力の際、空欄ではなくきちんと0を見せるために`NVL(e.cnt, 0)`を忘れないようにしてください。