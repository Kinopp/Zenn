...
```mermaid
flowchart TD
    subgraph Method1["例1：再帰的WITH句"]
        direction TB
        A1["アンカー部：社長を特定"] --> A2["再帰部：部下を次々に連結"]
        A2 --> A3["SEARCH DEPTH FIRST BY
        で深さ優先の順序を明示的に指定"]
    end
    subgraph Method2["例2：CONNECT BY"]
        direction TB
        B1["START WITH：ルートを指定"] --> B2["CONNECT BY PRIOR：
        親子関係を一行で定義"]
        B2 --> B3["標準で深さ優先探索
        （追加指定は最小限）"]
    end
...
| 方式 | 汎用性 | 柔軟性 |
| :--- | :--- | :--- |
| 再帰的WITH句 | PostgreSQL、SQL Serverでも動く | 複雑な計算を再帰中に組み込みやすい |
| CONNECT BY | Oracle専用（一部他DBで互換あり） | 短く慣れると素早く書ける |
...
flowchart TD
    A["① orgtree
    再帰でhierarchy_pathを構築
    100→100->101→100->101->108..."] --> B["② orgrollup
    パスの前方一致(LIKE)で
    自分の配下全員を特定し集計"]
    B --> C["③ 割合算出
    own_salary ÷ total_org_salary"]
    C --> D["④ LPADでインデント整形"]
```
...
flowchart LR
    ...
    A -->|"同様にヒット"| D["t2: 109
    「100->101->108->109」"]
    B -.->|"SUM(salary)"| E["101の配下全員の
    給与合計"]
    C -.-> E
    D -.-> E
```

例えば`t1`が「101」の場合、パスが「101...」で始まる人（101自身と、その部下全員）をすべてヒットさせ、その給与を合計しています。
...
```mermaid
flowchart TD
    A["STEP1: category_sales_raw
    カテゴリ×年で最小単位の集計"] --> B["STEP2: category_pivoted
    PIVOTで年を横に展開
    （縦持ち→横持ち）"]
    B --> C["STEP3: analyzed_data
    成長率とRATIO_TO_REPORTで
    シェアを計算"]
    C --> D["STEP4: 最終SELECT
    フィルタリング＋RANK付与
    ＋表示フォーマット整形"]
...
```mermaid
flowchart LR
    A["100 Steven"] -->|"本来はNULL
    →ループ偽装で102に書き換え"| B["102 Lex"]
    B -->|"元々の設定：
    上司は100"| A
    A --> C["101 Neena"]
    B --> D["103 Alexander"]
...
このクエリの重要なポイントは、`CONNECT BY`句の直後に置かれた`NOCYCLE`（ノーサイクル）というキーワードです。今回の検証データでは、社長である100番（Steven）のマネージャーが102番（Lex）に設定されている一方、102番のマネージャーは100番です。通常の`CONNECT BY`だと、`100 → 102 → 100 → 102 → ...`と永遠に巡回し続け、最終的にデータベースが処理を中断すべきだと判断して`ORA-01436`エラーを出しクエリを強制終了させてしまいます。

```mermaid
flowchart TD
    A["CONNECT BY（NOCYCLEなし）"] --> B["100→102→100→102→...
    永遠に巡回"] --> C["ORA-01436エラー
    クエリ強制終了"]
    D["CONNECT BY NOCYCLE"] --> E["過去に通った行(100)に
    戻ると察知"] --> F["その枝の探索を安全に打ち切り
    エラーなしで完結"]
```

`CONNECT BY NOCYCLE`と記述しておくと、Oracleは内部で「一度通ったルート（履歴）」を記憶しながら探索し、「この先に進むと過去に通った行（100番）に戻ってしまう」と察知した瞬間に、エラーを出さずにその枝の探索を安全に打ち切ってくれます。

安全にクエリが完結したら、次は「どこにバグがあるか」を見つける必要があります。そこで役立つのが2つの擬似列です。

| 擬似列 | 判定内容 | 今回の該当行 |
| :--- | :--- | :--- |
| `CONNECT_BY_ISCYCLE` | この行の次に進むと過去のデータに戻ってループが完成する | `102`（Lex）→`IS_LOOP=1` |
| `CONNECT_BY_ISLEAF` | それより下に部下が一人もいない末端（葉） | `101`（Neena）、`104`（Bruce）→`IS_LEAF=1` |

`CONNECT_BY_ISCYCLE`は「この行の次に進むと、過去のデータに戻ってループが完成してしまう」という、ループの引き金（発生源）になっている行にだけ1を立てます。期待する結果の`EMPLOYEE_ID = 102`を見てください。「100（Steven）→102（Lex）→（次に行くと100に戻る）→ループ発生」という構造で、102番の行で`IS_LOOP`が1になっています。これを見つけるだけで、管理者は「102番のマネージャー設定（または100番のマネージャー設定）が間違っている」と原因を特定できます。
