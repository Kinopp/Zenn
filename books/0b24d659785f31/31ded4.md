...
WITH monthly_kpi AS (
    ...
    SELECT '2026-03', 105 FROM dual UNION ALL
    SELECT '2026-04', 120 FROM dual UNION ALL
    SELECT '2026-05', 120 FROM dual UNION ALL
    SELECT '2026-06', 90  FROM dual UNION ALL
    SELECT '2026-07', 95  FROM dual UNION ALL
    SELECT '2026-08', 100 FROM dual UNION ALL
    SELECT '2026-09', 100 FROM dual UNION ALL
    SELECT '2026-10', 80  FROM dual UNION ALL
    SELECT '2026-11', 85  FROM dual
)
SELECT
...
FROM monthly_kpi
    MATCH_RECOGNIZE (
        ORDER BY month
        MEASURES
            FIRST(month)         AS start_month,  -- 起点(strt)行の月
            LAST(month)          AS end_month,     -- 最後にマッチした行の月
            COUNT(changed.month) AS total_changes, -- upとdown合わせた変化の総数
            COUNT(up.month)      AS up_count,
            COUNT(down.month)    AS down_count
        ONE ROW PER MATCH
        AFTER MATCH SKIP TO LAST changed
        -- パターン：起点の後、上昇か下降のどちらかが3回以上連続
        PATTERN ( strt (up|down){3,} )
        -- 上昇(up)と下降(down)を、changedという1つの集合名でまとめる
        -- ※SUBSET句は必ずPATTERNの後、DEFINEの前に書く
        SUBSET changed = (up, down)
        DEFINE
            up   AS revenue > PREV(revenue),
...
## 解説
これまでの問題では、`up`や`down`のようなパターン変数を`MEASURES`で使う際、それぞれ個別に`FIRST(up.month)`・`LAST(down.month)`のように**変数ごとに**参照してきました。しかし今回のように「上昇か下降かは問わず、とにかく"何か変化があった"月の範囲や件数を知りたい」というケースでは、変数ごとに書いていると意外と面倒なコードになります。ここで登場するのが`SUBSET`句です。

```mermaid
flowchart LR
    A["DEFINE up
    revenue > PREV(revenue)"] --> C["SUBSET changed = (up, down)"]
    B["DEFINE down
    revenue < PREV(revenue)"] --> C
    C --> D["MEASURES内で
    changed.* として
    まとめて集計可能に"]
```
...
SUBSET changed = (up, down)
```
`SUBSET`句は、`DEFINE`で定義した複数のパターン変数を、**1つの新しい名前（集合名）にまとめる**機能です。今回は`up`と`down`という2つの変数を`changed`という名前にまとめています。これによって、`MEASURES`句の中で`changed.month`のように参照すると、「`up`か`down`のどちらかに分類された行すべて」を対象にした集計ができるようになります。

```sql
PATTERN ( strt (up|down){3,} )
```
`PATTERN`自体には`changed`という名前は直接書けない点に注意してください。あくまで`PATTERN`で書けるのは実際に`DEFINE`した変数（`up`・`down`）だけです。`(up|down){3,}`は16-11で学んだ`PERMUTE`とは違う、**単純な論理OR（`|`）をグループ化して量化子`{3,}`を付ける**という書き方で、「`up`か`down`のどちらかが3回以上連続する」ことを表しています。`SUBSET`はあくまで`MEASURES`（および`DEFINE`内のナビゲーション関数）で使うための"後から作る別名"だと理解しておいてください。

### SUBSETを使わないと何が面倒なのか
もし`SUBSET`を使わずに同じ結果を得ようとすると、以下のように書く必要があります。
...
MEASURES
    FIRST(month) AS start_month,
    LAST(month)  AS end_month,
    COUNT(up.month) + COUNT(down.month) AS total_changes,
    COUNT(up.month)   AS up_count,
    COUNT(down.month) AS down_count
```
`COUNT`のような単純な集計であれば`COUNT(up.month) + COUNT(down.month)`のように**足し算で代用**できてしまいます。しかし、これが`FIRST`や`LAST`だった場合はそう簡単にはいきません。例えば「`up`か`down`のどちらか早い方の月」を求めようとすると、`up`が1件も出現しない可能性を考慮して以下のような複雑な式が必要になります。

```sql
-- SUBSETなしで「upかdownの、より早い方の月」を求める場合
LEAST(
    NVL(FIRST(up.month), FIRST(down.month)),
    NVL(FIRST(down.month), FIRST(up.month))
)
```
一方`SUBSET`を使えば、単に`FIRST(changed.month)`と書くだけで、Oracleが内部で「`up`か`down`いずれかに分類された行の中で、時系列上最初のもの」を正しく解決してくれます。

| やりたいこと | SUBSETなし | SUBSETあり |
| :--- | :--- | :--- |
| 変化した月の合計数 | `COUNT(up.month) + COUNT(down.month)` | `COUNT(changed.month)` |
| 変化した最初の月 | `LEAST(NVL(...), NVL(...))`のような複雑な式 | `FIRST(changed.month)` |
| 変化した最後の月 | 同様に複雑な式が必要 | `LAST(changed.month)` |

`COUNT`のような単純な合算では恩恵が薄いものの、`FIRST`/`LAST`のような「複数変数を横断した順序」を扱う集計になった途端、`SUBSET`の価値がはっきりと出てきます。

### AFTER MATCH SKIP TO でもSUBSET名が使える
```sql
AFTER MATCH SKIP TO LAST changed
```
`SKIP TO LAST`の対象にも、`SUBSET`で定義した集合名を指定できます。「`up`であろうと`down`であろうと、とにかく最後に変化があった行まで飛ばす」という意味になり、`up`と`down`のどちらで終わったかを気にせず書けるのが利点です。もしここで`SKIP TO LAST up`のように片方の変数名だけを固定してしまうと、マッチが`down`で終わった場合に正しく機能しません。

### 結果の確認
```mermaid
flowchart TD
    A["01月:100(strt)"] --> B["02月:110 UP"]
    B --> C["03月:105 DOWN"]
    C --> D["04月:120 UP"]
    D -.->|"05月はFLATで
    チェーンが途切れる"| E["Match1確定
    (02〜04月、UP2/DOWN1)"]
    F["05月:120(strt)"] --> G["06月:90 DOWN"]
    G --> H["07月:95 UP"]
    H --> I["08月:100 UP"]
    I -.->|"09月はFLATで
    チェーンが途切れる"| J["Match2確定
    (06〜08月、UP2/DOWN1)"]
```
1つ目のマッチは、01月を起点に02月（UP）・03月（DOWN）・04月（UP）と3ヶ月連続で変化が続き、05月が横ばい（FLAT、前月と同額）になった時点でチェーンが途切れます。`TOTAL_CHANGES=3`・`UP_COUNT=2`・`DOWN_COUNT=1`という結果は、`SUBSET`によって上昇・下降を区別せず「変化」としてまとめて数えつつ、内訳も個別に取得できていることを示しています。09月の横ばいの後（10月DOWN、11月UPの2件のみ）は3ヶ月連続という条件を満たせないため、3つ目のマッチは成立していません。

:::message
### MATCH_RECOGNIZE句の記述順序（節の並び）
`MATCH_RECOGNIZE`句を構成する各節には、以下の決まった記述順序があります。
...
| 2 | `ORDER BY` | 必須 |
| 3 | `MEASURES` | 任意 |
| 4 | `ONE ROW`\|`ALL ROWS PER MATCH` | 任意 |
| 5 | `AFTER MATCH SKIP TO ...` | 任意 |
| 6 | `PATTERN` | 必須 |
| 7 | `SUBSET` | 任意（`PATTERN`使用時のみ有効） |
| 8 | `DEFINE` | 必須 |

`SUBSET`は「`PATTERN`で定義した変数を、後からグルーピングする」という性質上、**必ず`PATTERN`より後**に書く必要があります。`MEASURES`句の中で`changed.month`のように`SUBSET`名を使っているため、一見「`SUBSET`は`MEASURES`より前で定義しておくべきでは」と直感的に感じるかもしれませんが、`MATCH_RECOGNIZE`全体はSQLパーサーが構文全体を解析した上で評価されるため、**文中の記述位置と評価される順序は必ずしも一致しません**。文法上のルールとして「`PATTERN`の後・`DEFINE`の前」と機械的に覚えておくのが確実です。
:::

### SUBSETは「新しい判定ロジック」ではなく「後付けの別名」
`SUBSET`に初めて触れると、「`changed`という新しい条件を`DEFINE`しているのでは」と誤解しがちですが、そうではありません。`SUBSET`はあくまで**すでに`DEFINE`で判定が終わった変数を、後から集計しやすいようにグルーピングする**ための仕組みです。判定ロジックそのものは、あくまで個々の`DEFINE`（今回なら`up`と`down`）が担っています。「`SUBSET`で新しい条件を作れるわけではない」という点を押さえておくと、構文を誤解せずに済みます。

### 実務での応用例
| 分野 | まとめて扱いたい複数の変数の例 |
| :--- | :--- |
| 与信・リスク管理 | 「延滞」と「限度額超過」という異なる異常を、まとめて「与信アラート」として期間集計 |
| 品質管理 | 「上限逸脱」と「下限逸脱」という別種の異常を、まとめて「規格外」として発生期間を集計 |
| システム監視 | 「CPU高負荷」と「メモリ高負荷」という別種の警告を、まとめて「リソース逼迫期間」として集約 |

このように、`DEFINE`では意味的に区別して定義しておきたいが、`MEASURES`側では「まとめて1つの事象」として扱いたい、という場面は実務でよくあります。`SUBSET`は、`DEFINE`の粒度の細かさと`MEASURES`での集計のしやすさを両立させるための、地味ながら便利な機能です。
