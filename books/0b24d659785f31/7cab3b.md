...
# 【完全版】問題15-12：IGNORE NAV / KEEP NAVによるNULL値の扱い
### 難易度：★★★☆☆ (Lv.3)
## 問題
ある販売代理店の月次インセンティブ（歩合給）データを管理しています。**2月は対象キャンペーンが実施されなかった月**であり、本来は歩合給`0`円として計上されるべきところ、システムの都合上、金額欄が`NULL`のまま登録されてしまっています。

以下のサンプルデータをもとに、**第1四半期（1〜3月）の歩合給累計額**を算出し、`202203`の行の`CUM`列にセットしてください。`NULL`の月についても`0`円として扱い、累計額が正しく計算されるようにしてください。

## サンプルデータ
```sql
WITH bonus_data AS (
    SELECT 202201 AS tm, 1000000 AS sm FROM dual UNION ALL
    SELECT 202202, CAST(NULL AS NUMBER) FROM dual UNION ALL -- キャンペーン未実施のためNULLで登録
    SELECT 202203, 1200000 FROM dual
)
...
| 202201 | 1000000 |     |
| 202202 |    |    |
| 202203 | 1200000 | 2200000 |

## NG例（よくある間違い）
```sql:NG例：オプションを何も指定しない場合
WITH bonus_data AS (
    SELECT 202201 AS tm, 1000000 AS sm FROM dual UNION ALL
    SELECT 202202, CAST(NULL AS NUMBER) FROM dual UNION ALL
    SELECT 202203, 1200000 FROM dual
)
SELECT
    tm,
...
MODEL
    DIMENSION BY ( tm )
    MEASURES ( sm,
               CAST(NULL AS NUMBER) AS cum )
    RULES (
        cum[202203]= sm[202201]+ sm[202202]+ sm[202203]
    )
ORDER BY
    tm
```
このクエリはエラーにはなりません。しかし結果は`CUM`列が3行とも`NULL`になってしまいます。`sm[202202]`が`NULL`であるため、「確定額（100万円）＋不明（NULL）＋確定額（120万円）」という足し算全体が、SQLの標準的な仕様どおり`NULL`に汚染されてしまうためです。件数の多い実データでこれが起きると、「システムは正常に動いているのに、なぜか集計結果だけがNULLになる」という原因の分かりにくい不具合として現れます。
...
## 解説
`MODEL`句には、ルールの右辺でセルを参照した際に、その値が`NULL`（または行自体が存在しない欠損セル）だった場合にどう扱うかを制御する`IGNORE NAV`／`KEEP NAV`という2つのオプションがあります（`NAV`は Non-Available Values の略です）。

| オプション | NULL・欠損セルの扱い | デフォルトかどうか |
| :--- | :--- | :---: |
| `KEEP NAV` | `NULL`はそのまま`NULL`として扱う（通常のSQLと同じ） | ⭕（省略時はこちら） |
| `IGNORE NAV` | 数値型は`0`、文字列型は空文字など、型ごとの既定値に自動変換してから計算 | ー |

何も指定しなければ`KEEP NAV`が適用されるため、NG例のように`NULL`を含む足し算は素直に`NULL`として伝播します。これは「NULLはNULLのまま扱う」というANSI SQL標準に沿った、ある意味で"正直"な挙動です。

```mermaid
flowchart LR
    A["sm[202201]
    1,000,000"] --> D{"KEEP NAV
    （デフォルト）"}
    B["sm[202202]
    NULL"] --> D
    C["sm[202203]
    ...
    B -->|"NULLを0とみなす"| F
    C --> F
    F -->|"0として計算に参加"| G["CUM = 2,200,000"]
```

`IGNORE NAV`を`MEASURES`句の直後（`RULES`句の手前）に1行加えるだけで、`sm[202202]`の`NULL`は数値型の既定値である`0`として扱われるようになり、`cum[202203]`は`1000000 + 0 + 1200000 = 2200000`と正しく計算されます。

ここで重要な注意点が2つあります。

**1つ目は、「行自体が存在しない欠損セル」にも同じルールが適用されるということです。** 問題15-10では`PRESENTV`を使って「行が存在するかどうか」を判定しましたが、`IGNORE NAV`はそれとは別に、「存在しない行を計算に使おうとしたときに、エラーにせず既定値で計算を続行させる」という役割も持っています。`NULL`のセルと欠損しているセルは、`IGNORE NAV`の観点では同じ土俵で扱われる点を覚えておいてください。

**2つ目は、`IGNORE NAV`が常に"正解"とは限らないということです。** 今回は「キャンペーン未実施＝実質0円」という業務上の意味付けがあったからこそ、NULLを0として扱うことが正しい判断でした。もし`NULL`が「まだ集計されていない・未確定」という意味であれば、それを0円扱いしてしまうのは、逆に実態を隠してしまう危険な処理になります。`IGNORE NAV`を使う前に、「そのNULLは"ゼロ"を意味するのか、それとも"不明"を意味するのか」を必ず業務要件として確認する必要があります。

実務では、月次バッチ集計で一部の店舗・拠点からのデータ連携が未達だった場合に、その拠点を「実質ゼロ」として扱って全体集計を止めない、といった耐障害性の高い集計処理に`IGNORE NAV`が使われます。一方で、金額の正確性がクリティカルな会計処理などでは、あえて`KEEP NAV`（デフォルト）のまま`NULL`を伝播させ、「異常値が混入していることに気づける」状態を保つ、という設計判断もあり得ます。

---
<br><br>

# 【完全版】問題15-13：AUTOMATIC ORDERとSEQUENTIAL ORDER（ルール評価順序の制御）
### 難易度：★★★★☆ (Lv.4)
## 問題
SHスキーマの`SALES`テーブルを使用し、2022年10月〜12月の実績値をもとに、**2023年1月の予測値（10月実績×1.1）** と、**その予測値をもとにした対10月比の成長率（％）** を算出してください。

**【算出ルール】**
* **予測値（SM）**：2023年1月 = 2022年10月の実績 × 1.1（小数点第3位を切り捨て）
* **成長率（GRW）**：`(2023年1月の予測値 － 2022年10月の実績) ÷ 2022年10月の実績 × 100`（小数点第3位を四捨五入）
...
## NG例（よくある間違い）
```sql:NG例：業務上わかりやすい順にルールを書いてしまった場合
SELECT
    tm,
    sm,
    grw
...
MODEL
    DIMENSION BY ( tm )
    MEASURES ( sm,
               CAST(NULL AS NUMBER) AS grw )
    RULES (
        -- 「まず成長率を出したい」という業務上自然な順序で書いた
        grw[202301]= ROUND(( sm[202301]- sm[202210]) / sm[202210]* 100, 2),
        sm[202301]= TRUNC(sm[202210]* 1.1, 2)
    )
ORDER BY
    tm
```
このクエリもエラーにはならず、`sm[202301]`（予測値）は正しく`2694084.70`が算出されます。しかし`grw[202301]`は **`NULL`のまま** になってしまいます。理由は、`RULES`句が何も指定しなければ「書かれた順番どおり」に実行される`SEQUENTIAL ORDER`という規定の動きをするためです。1行目の成長率ルールが実行される時点では、まだ2行目の予測ルールが実行されておらず`sm[202301]`は存在しないため、`grw[202301]`はNULLを参照した計算結果としてNULLになってしまいます。
...
## 解説
`MODEL`句の`RULES`句には、複数のルールをどの順番で実行するかを制御する`AUTOMATIC ORDER`／`SEQUENTIAL ORDER`という指定があります。

| 指定 | ルールの実行順序 | デフォルトかどうか |
| :--- | :--- | :---: |
| `SEQUENTIAL ORDER` | **書かれた順番**どおりに、上から1行ずつ実行 | ⭕（省略時はこちら） |
| `AUTOMATIC ORDER` | ルール同士の**依存関係**をOracleが解析し、必要な順序を自動決定してから実行 | ー |

これまでの15-1〜15-12の問題は、たまたますべて「依存関係の順序」と「書いた順序」が一致していたため、この違いが表面化しませんでした。しかし今回のように「成長率（結果）を先に書きたい」という自然な文章の流れと、「予測値を先に計算しないと成長率が出せない」という計算上の依存関係が食い違うケースでは、両者の違いが顕在化します。

```mermaid
flowchart TD
    subgraph SEQUENTIAL["SEQUENTIAL ORDER（デフォルト）"]
        direction TB
        A1["①grw[202301]を計算
        → sm[202301]はまだ存在せずNULL
        → grw[202301] = NULL"] --> A2["②sm[202301]を計算
        → 2694084.70が作られる
        （grw側はもう手遅れ）"]
    end
    subgraph AUTOMATIC["AUTOMATIC ORDER"]
        direction TB
        B1["①依存関係を解析
        grw[202301]はsm[202301]に依存"] --> B2["②sm[202301]を先に計算
        → 2694084.70"]
        B2 --> B3["③grw[202301]を計算
        → 正しく10.00%"]
    end
```

`AUTOMATIC ORDER`を指定すると、Oracleは各ルールの右辺を解析し、「このルールはあのルールの計算結果に依存している」という依存グラフを組み立てた上で、書かれた順序に関係なく正しい順序で計算を行います。今回はコードを一切並び替えることなく、`RULES`の直後に`AUTOMATIC ORDER`という一言を加えるだけで、`grw[202301]`が正しく`10`（％）と算出されるようになります。

なお、`AUTOMATIC ORDER`には代償もあります。複数のルールが同じセルに対して矛盾した代入をしようとした場合、`SEQUENTIAL ORDER`であれば「後に書かれた方で単純に上書きされる」だけで済みますが、`AUTOMATIC ORDER`では「どちらを先に評価すべきか一意に決まらない」としてエラーになります。依存関係が循環している場合（AがBに依存し、BもAに依存する）も同様にエラーとなり、実行前に矛盾が検出されるという安全側の設計になっています。

| 指定 | メリット | 注意点 |
| :--- | :--- | :--- |
| `SEQUENTIAL ORDER`（デフォルト） | 動きが直感的で予測しやすい（上から順に実行されるだけ） | ルールの記述順序を、計算の依存順序に**自分で**合わせる必要がある |
| `AUTOMATIC ORDER` | 依存関係をOracleに任せられるため、記述順序を気にしなくてよい | 同一セルへの多重代入や循環依存があるとエラーになる |

実務では、ルールの本数が数十〜数百本規模になる大規模な`MODEL`句（在庫シミュレーションや連結決算の仕訳展開など）で特に効果を発揮します。ルールの数が増えるほど、「正しい依存順序どおりに手作業で並べる」のは現実的でなくなっていくため、`AUTOMATIC ORDER`に依存関係の解決を任せてしまう設計が好まれる傾向があります。一方で、今回のように少数精鋭のルールで、かつ意図的に「同じセルを段階的に上書きしていく」ような処理（15-9のFOR句を使った補正など）をしたい場合は、`SEQUENTIAL ORDER`（デフォルト）の"書いた順に実行される"という素直さの方が扱いやすいこともあります。
