...
### 3. 集計関数の相棒「GROUP BY」と「HAVING」
集計関数は単体で使うこともありますが、多くは **`GROUP BY`** と組み合わせて「グループごと」の数値を計算します。
...
HAVING 
    AVG(salary) > 5000; -- 集計後の結果で絞り込む（WHEREは集計前）
```
...
# 問題8-3（集計後の条件）  *Lv2*
### **COスキーマの`ORDERS`テーブルから年月ごとの注文数のうち、月の注文数が150以上の年月を算出してみましょう。ただし、キャンセルされた注文（`ORDER_STATUS`が「CANCELLED」のもの）は、集計対象から外すようにしてください。（`ORDER_TMS`の昇順）**
...
#### ① 重複しているデータだけを抽出する
「同じメールアドレスが2回以上使われているユーザーを探す」といった処理は `HAVING` の独壇場です。

```sql
SELECT email, COUNT(*)
FROM users
GROUP BY email
HAVING COUNT(*) > 1; -- 2件以上存在するグループだけを表示
```

#### ② 特定の条件を満たすメンバーが複数いるグループを探す
...
FROM employees
WHERE job_id = 'IT_PROG' -- まずプログラマーだけに絞る
GROUP BY department_id
HAVING COUNT(*) >= 3;    -- その人数が3人以上の部署だけ残す
```
> **ちょっとしたコツ：**
> どちらでも書ける場合（例：`WHERE department_id = 10` か `HAVING department_id = 10` か）は、**`WHERE` に書くほうが圧倒的に速い**です。集計する前にデータを減らしたほうが、コンピュータの仕事が楽になるからです。
...
#### 基本構文
```sql
LISTAGG(列名, ', ') WITHIN GROUP (ORDER BY ...) AS ```
```
